...
// 2) Get books by author (optional genre)
@GetMapping("/books/author/{author}")
public ResponseEntity<Collection<Book>> getBooksByAuthor(
        @PathVariable String author,
        @RequestParam(required = false) String genre) {

    Collection<Book> books =
            libraryService.getBooksByAuthorAndGenre(author, genre);

    logger.info("The books retrieved for author " + author + " and genre " + genre);

    return new ResponseEntity<>(books, HttpStatus.OK);
}

// 3) Get books due on date
...
// 4) Check book availability date
